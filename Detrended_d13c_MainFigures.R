...
figure3 <- rbind(detrended_eauc1, detrended_eauc2, detrended_stf1) %>%
...
figure3$labels$colour <- "Coral"
figure3$labels$y <- expression(paste("Detrended Bulk ", "\u03B4" ^ "13", "C", " (\u2030)"))

figure3$labels$title <- expression(paste("Southwest Pacific Corals Detrended Bulk ", "\u03B4" ^ "13", "C", " Resampled Data"))

figure3

figure4 <- rbind(detrended_eauc1_raw, detrended_eauc2_raw, detrended_stf1_raw) %>%
...
figure4$labels$y <- expression(paste("Detrended Bulk ", "\u03B4" ^ "13", "C", " (\u2030)"))

figure4$labels$title <- expression(paste("Southwest Pacific Corals Detrended Bulk ", "\u03B4" ^ "13", "C", " Raw Data"))


figure4


final_figure3 <- ggarrange(figure3, figure4, 
          labels = c("A", "B"),
          ncol = 1, nrow = 2)
...
detrended_interp_all <- rbind(detrended_eauc1, detrended_eauc2, detrended_stf1) %>%
  rename(age = age_int, d13c = output)
